...
In the game's universe, the worlds are bigger, but the playable area is just a small neighborhood around the \emph{Hub}.
...
The \emph{Hub} should be several tiles away from the edges of the playable area, so the player has good access to the path segments near it.
...
It can be more off-center in levels with only one or two long paths, where the path can snake through the world from the side furthest away from the \emph{Hub}, also covering the world somewhat evenly.
...
Of course, this button only appears when the selected object is a building that it is not permanent, unlike for example, the \emph{Hub}.
