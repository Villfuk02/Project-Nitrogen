...
As can be seen in figure \ref{fig:pvz-fight}, the zombies come from the right side of the screen and advance left.
...
We can also see two \emph{Repeaters} in the upper left part of the picture, one of them shooting at a zombie.
...
In figure~\ref{fig:sts-fight} we can see the player character on the left, facing a \emph{Jaw worm} on the right of the screen.
...
The game we intend to make will be single-player.
...
The scale of the game as outlined in \ref{sec:original-vision} is quite large.
Furthermore, it would need a lot of content and polish before being able to be released as a full game.
However, we will create a functional demo version, which can be used to playtest the core gameplay.
The demo must be prepared for future development so that more content can be added later, but it will still contain some base content in order to be playable.

Our main goals will thus be:
\begin{enumerate}
    \item Design the game's mechanics and features.
    \item Implement the systems and mechanics necessary to play through a battle. As described in section \ref{sec:original-vision}, this includes attackers, towers, abilities, and the economy.
    \item Implement a system to procedurally generate the level a battle takes place~on.
    \item Include tutorial to explain the game's mechanics to the player \xxx{(might not happen)}.
    \item Run a playtest \xxx{(might not happen)}.
\end{enumerate}

The demo version will allow the player to progress through battles and collect blueprints, however, there will be no map view to let them choose their path.
