...
If custom highlights are needed, implement a custom script derived from \mono{HighlightProvider}.
...
\subsection{Attacker Behavior Script Example}\label{sec:docs-armored-skeleton}
...
\begin{flushleft}
    \begin{tabular}{@{}>{\setlength\parindent{1em}}p{0.6\textwidth}p{\dimexpr 0.4\textwidth-2\tabcolsep}@{}}
        ...
        \emph{Cooldown} determines how many waves the player has to wait after using the blueprint before they can use it again.
         ...
         & \raisebox{\dimexpr -1\height+2mm}{\includegraphics[width=0.92\linewidth]{img/Steps/blueprint inspector.png}}
    \end{tabular}
\end{flushleft}
...
Below is the inheritance tree of the scripts derived from \mono{Blueprinted}.
Scripts that implement the behavior of one specific blueprinted object are grouped together by their base class to save space.
\dirtree{%
    .1 \textbf{Blueprinted}.
    .2 \textbf{Ability}.
    .3 OrbitalLaser, Streamline.
    .3 \textbf{TargetedAbility}.
    .4 Catalyst, Greande, LightningStorm, Meteor.
    .2 \textbf{Building}.
    .3 Amplifier, EnergyHarvester, Radar, SolarPanel,\\SurfaceDrill.
    .3 \textbf{ProductionBuilding}.
    .4 DustRefinery, MatterReplicator.
    .3 \textbf{Tower}.
    .4 FrostbiteFrontier, HoloSiphon, Sledgehammer,\\StaticSparker, UltraRay.
    .4 \textbf{ProjectileTower}.
    .5 Galvanizer, Mortar.
    .5 \textbf{BasicProjectileTower}.
    .6 DoubleSentry, GatlingGun, Predator, Sniper.
}
\

As an example, section~\ref{sec:docs-meteor} explains the implementation of the ability \emph{Meteor}.
Similarly, section~\ref{sec:docs-sledgehammer} explains the implementation of the tower \emph{Sledgehammer}.
...
Below is a summary of the already implemented placement scripts, organized into an inheritance tree.
Abstract classes are in \emph{italics}.
\dirtree{%
    .1 \emph{Placement}.
    .2 GlobalPlacement\textrm{~--- Doesn't need a position or target.}.
    .2 SimplePointPlacement\textrm{~--- Place on any point in the world.}.
    .2 \emph{TilePlacement}\textrm{~--- Place on tiles.}.
    .3 \emph{BuildingPlacement}\textrm{~--- For buildings.}.
    .4 HeightBasedBuildingPlacement\textrm{~--- Limit valid tiles by height.}.
    .5 SolarPanelPlacement\textrm{~--- Used by \emph{Solar Panel}. Tells the solar panel at which height level it's being placed.}.
    .4 ObstacleSpecificBuildingPlacement\textrm{~--- Configure on which\\\-\ \ obstacles it can be placed, and whether it can be placed on\\\-\ \ slanted tiles.}.
    .5 SurfaceDrillPlacement\textrm{~--- Used by \emph{Surface Drill}. Tells the drill if it's being placed on minerals or fuel.}.
    .5 RotatableBuildingPlacment\textrm{~--- Also select a fixed orientation when placing.}.
    .4 SpacedBuildingPlacement\textrm{~--- Cannot be placed next to a building with the same original blueprint.}.
    .3 RotatableTilePlacement\textrm{~--- Also select a fixed orientation when\\placing.}.
}
\
...
\pagebreak
\dirtree{%
    .1 \emph{HighlightProvider}.
    .2 AttackerHighlightProvider\textrm{~--- Used by attackers, highlights only the attacker.}.
    .2 OrbiltalLaserHighlightProvider\textrm{~--- Used by \emph{Orbital Laser}.}.
    .2 \emph{PlacementHighlightProvider}\textrm{~--- Used by other highlight providers\\\-\ \ as described below.}.
    .3 PlaceAnywhereHighlightProvider\textrm{~--- For blueprints that can be\\placed anywhere.}.
    .3 TilePlacementHighlightProvider\textrm{~--- For blueprints that are placed\\\-\ \ on tiles.}.
    .4 \emph{BuildingAffectingHighlightProvider}\textrm{~--- For buildings that\\\-\ \ affect other buildings in some way.}.
    .5 AmplifierHighlightProvider\textrm{~--- Used by \emph{Amplifier}.}.
    .5 RadarHighlightProvider\textrm{~--- Used by \emph{Radar}.}.
    .2 TargetingHighlightProvider\textrm{~--- For blueprints that have a targeting and\\\-\ \ want to display their range.}.
    .3 RadiusHighlightProvider\textrm{~--- For blueprints that can target anything in a circle centered on their position.}.
    .2 TileHighlightProvider\textrm{~--- Used by tiles. Delegates work to the highlight provider of the building placed on the tile.}.
    .2 TowerHighlightProvider\textrm{~--- Used by towers, displays their range.}.
}
\
