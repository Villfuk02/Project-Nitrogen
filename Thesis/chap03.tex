...
As a compromise, we tried to select the slots randomly, but give slots with lower entropy a greater weight, so they are more likely to be selected.
...
We also use some shortcuts for the values it computes in the individual steps, only to save space.
...
\end{equation*}
We will refer to the total damage the towers would deal to an attacker with speed 1 as \emph{damage capacity} $C$, and
...
By equation~\ref{eqn:C}, $\sum_{t \in T} d_t \cdot l_t / (s g) = C/(s g)$, so we know that
...
The blue area represents the defenses which can beat the wave, and the orange area represents defenses that can't exist because they would violate inequality~\ref{eqn:RC}.
...
Figure~\ref{fig:model-3-s2} shows that fast attackers are especially problematic for towers with small path coverage $l_t$.
...
In figure~\ref{fig:model-3-g0.5} we can see that with smaller spacing between the attackers, damage rate becomes less effective.
A similar effect can be seen when we decrease the number of attackers.
As shown in figure~\ref{fig:model-3-n1}, when the wave contains only one attacker, damage rate has no effect.
On the other hand, when there are many attackers, the damage rate becomes much more important.
...
Thus, to describe the difficulty of a wave, we can't just use one number, because we need 2 variables to accurately describe which defenses can beat the wave and which cannot.
...
We can substitute this into equation~\ref{eqn:time-to-kill} to get the recurrence relation
\begin{equation*}
...
Otherwise, the derivation is the same as for \hyperref[sec:analysis-waves-finite]{model~3}, so the player will beat a wave if
\begin{equation*}
    C/s + (n-1)Rg \geq \sum_{k=1}^{n} Rt_k.
\end{equation*}
If we put all wave-dependent terms on the right side of the equation and rearrange, we get
\begin{equation*}
...
So, the fixed condition would be
\begin{equation*}
    C \geq s(Rt_1 + \sum_{k=2}^{n} \max(Rt_k - Rg, 0)).
\end{equation*}

If we substitute the definition of $t_k$, we get
\begin{equation*}
...
Finally, we can calculate the explicit solution for the summation to obtain that the player's defenses will beat a wave if and only if
\begin{equation}\label{eqn:beat-model-4}
    ...
    \right),
\end{equation}
where $n^*$ is the smallest $k$ between 1 and $n$, such that
\begin{equation*}
    \frac{
        1-\alpha-\beta^{sg}+\alpha\beta^{sg}+\alpha(1-\alpha)^k\beta^{sgk}
    }{
...
So, we can say the defenses will beat the wave if
\begin{equation*}
...
Value rate represents how much damage we expect the player's defenses to deal to an attacker per second.
Value capacity represents how much damage we expect the defenses to deal to an attacker during its travel from the path start to the Hub.
...
\xxx{ TODO: rEwRiTE --- specify all other conditions, rejection sampling, go from there}
...
To fix this, for the last batch of the wave, we select the maximum amount of attackers we calculated, instead of a random amount.
...
To remediate this, we add another condition when determining the valid attackers for the last batch of the wave.
...
We will add some more that naturally extend from these:
Since we want to use up most of the capacity, a natural requirement is also that

First, we select a random spacing.
Similarly to a sequential wave, we can filter out all the attacker types that are invalid given this spacing.
However, this time they can be different for every path.
We ban all attacker types whose minimum spacing is greater than the selected spacing.
Also, at least 3 of the type must fit within the path capacity plus the global capacity.
We also check that it is possible the current path capacity using this attacker type.
For serial waves, we checked that one attacker fits, but here we want to have more granular control over the wave value just by increasing or decreasing the number of attackers.
The difference in relative value between 3 and 4 attackers is not as big as between 1 and 2 attackers, so we will be able to use up the capacity more.

Then we select a random attacker for each path from the set of valid attackers for that path.
Now that we have a set of attackers, we have to run more tests to decide if it can be used.
So we enter a loop, where we do some tests, and if the set of attackers fails one of the tests, we change one attacker in the set and try again.
If all tests succeed, we then produce the finished wave and distribute the new rate and capacity accordingly.
But first, let's look at the tests:

Since we want to use up the capacity as much as possible, we also have to use up every path capacity.
So, for the first test, we calculate the minimum number of attackers the wave must have in order to use up every path capacity.
We then check if this count is even a viable option for this wave.
Specifically, we check whether this number of attackers would fit within the capacity, including the new rate and new capacity we still haven't assigned.
To do this, we first calculate the value of the attackers for each path and sum these values together.
We now compare this value to the total combined value of all the capacitys, including the new capacity.
Since we know how many attackers are in the wave, we can also convert the new rate to the maximum amount of extra capacity it will provide by multiplying it by the number of attackers minus one.
If the total value of the attackers is greater than the final capacity size, this attacker set is not valid.
This result tells us that we need at least some amount of attackers to fill each path capacity, but this amount would not fit in the final capacity, so we can't use it.

So we try again with a different set of attackers.
We don't want to change the attackers we don't have to, so we only select a new random attacker for the path that had the \enquote{most expensive} attacker.
The test failed, because some attacker had too much value compared to the rest, and its path's rate and capacity, so that is the one we change.
Specifically, we change the attacker of the path whose value in the previous test overshot its path capacity the most.

If the test succeeds, we have a minimum number of attackers the wave can have in order to use up every path capacity.
...
So, we calculate the maximum number of attackers these restrictions let us use, and then we check whether the value of this many attackers is more than the combined capacity in the same way as in the previous test.
...
So, we try a different set off attackers, but this time we switch out the \enquote{cheapest attacker}.

If both tests succeed, we have found a good enough set of attackers for this wave.
Now we just calculate the maximum number of attackers that will fit within the capacity, and the wave is finished.
However, we still have to somehow distribute the new rate and capacity to the individual paths.
...
What are their uses and how they work is explained well in \citetitle{johnston2018random}~\cite{johnston2018random} by Johnston.
...
This approach is also used in the Unity engine.
In Unity, the game logic has to be implemented in methods \mono{Update} and \mono{FixedUpdate} of our \mono{MonoBehaviour} scripts.
Unity then calls these methods during runtime in the game loop.
...
This means that Unity is build with the separation of game logic and visuals in mind.

- 20 ticks per second is enough

- interpolate positions and visuals on Update

- some visuals are game-speed agnostic, some are not

- I thought about making some custom mini-framework for this, but many of the simulated variables the visuals are based on should be handled on case-by-case basis

- interpolation examples: attackers, projectiles, ballistic projectiles

\section{Targeting Attackers}

During a battle, how will the towers determine which attackers are in range, and which attacker to shoot at?
...
Technically, a tower needs to know which attackers are in range and which one of them to target only right before it shoots.
...
Specifically, we think of our problem as collision detection: we want to know which attacker colliders collide with the tower's range collider.
...
- two parts - getting the data, and rendering it

- simple for simple shapes, but complicated for los checks

- uhh divide into squares and test each point

- 16x16 big, but we don't know the final resolution

- los checks are expensive

- make low res first and refine

- priority calculation

- the number of samples per frame is defined for each range visuals

- for simple shapes, also specify the distance to a boundary, ie abilities

- sending to the graphics card
