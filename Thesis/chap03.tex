...
It would be very inefficient to go through this whole process a thousand times per second, when reading a value that is not currently changing, as can happen when creating the range visualization for a tower.
So, we also cache the result of the modifiable query to save on computation.
The cache gets invalidated whenever a modifier is subscribed, unsubscribed, or whenever someone calls the function to explicitly invalidate the cache.
