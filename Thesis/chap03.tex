...
The blue area represents the defenses which can beat the wave, and the orange area represents defenses that don't exist because they would violate inequality~\ref{eqn:RB}.
...
This gives us
\begin{equation*}
    B \geq \sum_{k=1}^{N} n_k h_k s_k - \sum_{k=1}^{N} (n_k - 1)(R g_k + \lambda^{s_k g_k})s_k.
\end{equation*}
...
The current formula essentially allows dealing damage in advance to attackers that haven't spawned yet, because the therm $R g_k + \lambda^{s_k g_k}$ is not limited by the total health of the attacker.
To fix this, we need to introduce this limit, so we get
\begin{equation*}
    B \geq \sum_{k=1}^{N} n_k h_k s_k - \sum_{k=1}^{N} (n_k - 1)\min(R g_k + \lambda^{s_k g_k}, h_k)s_k.
\end{equation*}
...
\begin{equation*}
    B \geq \sum_{k=1}^{N} n_k h_k s_k - \sum_{k=1}^{N} (n_k - 1)\min(R g_k + \lambda^{s_k g_k}, h_k)s_k - \sum_{k=2}^{N} \min(R + \lambda^{s_k}, h_k)s_k.
\end{equation*}
...
\begin{equation}
    B \geq \sum_{k=1}^{N} n_k h_k s_k - \sum_{k=1}^{N} (n_k - 1)\min(R g_k + \lambda^{s_k g_k}, h_k)s_k - \sum_{k=2}^{N} \min(R, h_k)s_k.
\end{equation}
