...
\head{Model 2: Infinite Waves}{waves-infinite}
...
\head{Model 3: Finite Waves}{waves-finite}
...
From \hyperref[head:waves-single]{model~1} we know that if the whole wave was a single attacker, the player's defenses would deal $B/s$ damage in total.
...
\begin{equation}\label{eqn:beat-finite-wave-ugly}
    B \geq (h - R g)(n - 1)s + hs.
\end{equation}

We cannot quantify the difficulty of the wave as one number anymore.
...
In figure \xxx{TODO} we show the set of defenses that can beat a particular wave, and we show how it changes when the wave's parameters change.
The blue area represents the defenses which can beat the wave, and the orange area shows which defenses cannot exist, because of inequality~\ref{eqn:RB}.
...
$T_2$ consists of one tower with $d_t=10$ and $l_t=1.5$, which is an unusually small path coverage to showcase what happens when $l_t/(sg) < 1$ as shown in figure \xxx{TODO}.
