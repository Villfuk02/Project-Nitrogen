...
The closeness is some function that decreases with distance, similar to \emph{crowding penalty} we used in section~\ref{sec:analysis-our-simulated-annealing} when generating paths.
...
For each phase, it will through all tiles, and for each tile, it will assign an obstacle to it with the probability that is calculated for that obstacle for that tile.
...
For example, we don't have to go through all tiles, only those that don't have an obstacle, or a path going through them.
...
However, this doesn't help us make smooth transitions, because we cannot distinguish points that are just outside a tile with the obstacle from positions far away.
...
\subsection{Quantifying Wave Difficulty}

In this section we aim to find a way to express the difficulty of a wave, that is as simple as possible, but it is close enough for the waves to not feel unfair.
...
In other words, that the wave was beatable, they just weren't prepared for it.
...
It will be the responsibility of the map generator to select the right wave difficulty based on the parameters it selected for world.
...
\head{Model 1~--- Single Attacker}{waves-single}
...
The difficulty of a wave can then be defined as the minimum strength $S$ of the player's defenses that is required to beat the wave.
...
\begin{equation*}
    \sum_{t \in T} d_t \cdot l_t / s \geq h.
\end{equation*}
...
Next, we will also take a look at a case with infinite attackers.

\head{Model 2~--- Infinite Attackers}{waves-infinite}
...
Intuitively, the player needs to kill each attacker in $g$ seconds or less in order to beat the wave.

As before, each tower $t$ can deal at most $d_t \cdot l_t / s $ damage to a single attacker.
However, now we're dealing with an infinite wave of attackers.
Either the tower can kill each attacker, in which case the defenses are strong enough to beat the wave.
Or, there will be some $n$th attacker the tower isn't able to kill.

Assume the region of path that is within the range of $t$ is continuous, that is, each attacker both enters the range and leaves it exactly once, except for the first $n-1$, which die before they leave the tower's range.
When the $n$th attacker leaves the tower's range, the tower starts dealing the damage to the $n + 1$st attacker, assuming it already is within range.
It can only deal damage to the next attacker for at most $g$ seconds, because it is $g$ seconds behind the previous attacker, so it will also leave the range soon.
The same holds for any other attacker after the $n+1$st.
