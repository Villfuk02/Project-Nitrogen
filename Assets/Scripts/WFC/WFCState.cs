...
public class WFCState
{
    ...
    readonly HashSet<WorldUtils.TerrainTypes>[,] terrainTypes;
    ...
    public WFCState()
    {
        ...
        terrainTypes = new HashSet<WorldUtils.TerrainTypes>[WorldUtils.WORLD_SIZE.x + 2, WorldUtils.WORLD_SIZE.y + 2];
        for (int x = 0; x < heights.GetLength(0); x++)
        {
            for (int y = 0; y < heights.GetLength(1); y++)
            {
                ...
                terrainTypes[x, y] = new HashSet<WorldUtils.TerrainTypes>((WorldUtils.TerrainTypes[])System.Enum.GetValues(typeof(WorldUtils.TerrainTypes)));
            }
        }
    }
    public WFCState(WFCState original)
    {
        ...
        terrainTypes = (HashSet<WorldUtils.TerrainTypes>[,])original.terrainTypes.Clone();
        for (int x = 0; x < heights.GetLength(0); x++)
        {
            for (int y = 0; y < heights.GetLength(1); y++)
            {
    ...
    //TERRAIN TYPES
    public HashSet<WorldUtils.TerrainTypes>[] GetValidTypesAtSlot(int x, int y)
    {
        HashSet<WorldUtils.TerrainTypes>[] ret = new HashSet<WorldUtils.TerrainTypes>[4];
    ...
    private HashSet<WorldUtils.TerrainTypes> GetValidTypesAtTile(int x, int y)
    {
        x++;
        y++;
    ...
    public void SetValidTypesAtSlot(int x, int y, HashSet<WorldUtils.TerrainTypes>[] h)
    {
    ...
    private void SetValidTypesAtTile(int x, int y, HashSet<WorldUtils.TerrainTypes> t)
    {
        x++;
        y++;
        if (!WorldUtils.IsInRange(x, y, terrainTypes.GetLength(0), terrainTypes.GetLength(1)))
