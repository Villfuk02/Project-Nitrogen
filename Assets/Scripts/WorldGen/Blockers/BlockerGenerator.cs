...
namespace WorldGen.Blockers
{
    public class BlockerGenerator : MonoBehaviour
    {
        ...
        public void PlaceBlockers(Vector2Int[] pathStarts, int[] pathLengths)
        {
            ...
            RegisterGizmos(StepType.Phase, () => new List<Vector2Int>(pathStarts) { WorldUtils.ORIGIN }.Select(p => new GizmoManager.Cube(Color.yellow, WorldUtils.TileToWorldPos(p), 0.5f)));
            ...
            foreach (var pos in tilesLeft_.AllEntries)
            {
        ...
        {
            RegisterGizmos(duration, () => tilesLeft_.AllEntries.Select(pos => new GizmoManager.Cube(Color.cyan, WorldUtils.TileToWorldPos(pos), 0.3f)));
            RegisterGizmos(duration, () =>
            {
                ...
                foreach (var tile in Tiles)
                {
                    ...
                    gizmos.AddRange(
                        tile.neighbors.Where(n => n is not null).Select(n => WorldUtils.TileToWorldPos(n.pos))
