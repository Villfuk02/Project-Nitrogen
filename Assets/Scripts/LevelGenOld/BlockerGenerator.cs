    ...
    public class BlockerGenerator : LevelGeneratorPart
    {
            ...
            for (int x = 0; x < WorldUtils.WORLD_SIZE.x; x++)
            {
                for (int y = 0; y < WorldUtils.WORLD_SIZE.y; y++)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        if (p.x >= 0 && p.y >= 0 && p.x < WorldUtils.WORLD_SIZE.x && p.y < WorldUtils.WORLD_SIZE.y && WFC.WFCGenerator.state.GetValidPassagesAt(pp.x, pp.y, i % 2 == 1).passable)
                        {
        ...
        void TryPlace(Vector2Int pos, bool force)
        {
            ...
            for (int i = 0; i < available.Count; i++)
            {
                ...
                bool valid = true;
                if (!b.onSlants && !WFC.WFCGenerator.state.GetTile(pos + Vector2Int.one).slants.Contains(WorldUtils.Slant.None))
                {
                    probability = 0;
                    valid = false;
                }
